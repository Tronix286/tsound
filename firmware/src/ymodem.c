...
/* Private functions ---------------------------------------------------------*/
extern void SeekAndReadWholeTrack(void);
#define MAX_DELTAS (32768L)
/**
...
{
  int tmp_c;
  tmp_c = getchar_timeout_us(timeout);
  if (tmp_c == PICO_ERROR_TIMEOUT)
    return -1;
  *c = tmp_c;
  return 0;
}
    ...
    do
    {
      ...
      if ((Receive_Byte(&receivedC[0], 100000) == 0)  && (receivedC[0] == ACK))
      {
        ...
        if (size > pktSize)
        {
           ...
           size -= pktSize;
           blkNumber++;
           if (blkNumber % (MAX_DELTAS/1024) == 0)
           {
             SeekAndReadWholeTrack();
             buf_ptr = buf;
           }
        }
        else
