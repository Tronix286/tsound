...
static PSG* createPSG(int psgClock, int sampleRate)
{
  ...
  //PSG_set_quality(psg, 1);   // Quality = high?
  PSG_reset(psg);
  return psg;
}
...
void audioUpdate()
{
  ...
  uint16_t leftAudio = (covox_val*64+ abs((((int32_t)psg0->ch_out[0] + (int32_t)psg1->ch_out[1] + (int32_t)psg0->ch_out[2] + (int32_t)psg1->ch_out[2])))) * PWM_WRAP >> 14;
  uint16_t rightAudio = (covox_val*64 +abs((((int32_t)psg1->ch_out[0] + (int32_t)psg0->ch_out[1] + (int32_t)psg0->ch_out[2] + (int32_t)psg1->ch_out[2])))) * PWM_WRAP >> 14;
